...
def load():
    ...
    """
    classifier = RF() # Default hyperparameters are optimal ones.

    model_base_path = os.path.join(os.path.dirname(models.__file__), "../..")

    clf_path = os.path.join(
        model_base_path, "pretrained/random_forest_classifier.pkl")
    vect_path = os.path.join(
        model_base_path, "pretrained/random_forest_vectorizer.pkl")

    if os.path.exists(clf_path) and os.path.exists(vect_path):
        classifier = RF.load(vect_path, clf_path)
    else:
