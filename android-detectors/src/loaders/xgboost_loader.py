...
def load():
    ...
    """
    classifier = XGBoost() # Default hyperparameters are optimal ones.

    model_base_path = os.path.join(os.path.dirname(models.__file__), "../..")

    clf_path = os.path.join(
        model_base_path, "pretrained/xgboost_classifier.pkl")
    vect_path = os.path.join(
        model_base_path, "pretrained/xgboost_vectorizer.pkl")

    if os.path.exists(clf_path) and os.path.exists(vect_path):
        classifier = XGBoost.load(vect_path, clf_path)
    else:
